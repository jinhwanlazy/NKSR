...
void pybind_kernel_eval(py::module& m) {
    ...
    m.def("kernel_evaluation_grad", [](const fvdb::GridBatch& grid,
        ...
        TORCH_CHECK(gradKernelQuery.edim() == 2);
        TORCH_CHECK(gridAlpha.element_count() == grid.total_voxels(), "grid_alpha must have number of voxels");
        auto res = KernelEvaluation::apply(
                grid.impl(), true,
    ...
    m.def("rhs_evaluation", [](const fvdb::GridBatch& grid,
        ...
        TORCH_CHECK(gradKernelPts.edim() == 2);
    ...
    m.def("csr_matrix_multiplication", [](
        ...
        TORCH_CHECK(coordsI.element_count() == grid_i.total_voxels());
        TORCH_CHECK(coordsJ.element_count() == grid_j.total_voxels());
        TORCH_CHECK(iValue.element_count() == iColInds.element_count());
        TORCH_CHECK(jValue.element_count() == jColInds.element_count());
