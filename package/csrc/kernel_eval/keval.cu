...
variable_list KernelEvaluation::forward(
    ...
    torch::Tensor outFunc = torch::zeros(query.element_count(), opts);
    torch::Tensor outGradFunc = torch::zeros({0, 3}, opts);
    if (grad) {
        outGradFunc = torch::zeros({query.element_count(), 3}, opts);
    }
...
variable_list KernelEvaluation::backward(AutogradContext *ctx, variable_list grad_output) {
    ...
    auto query = fvdb::JaggedTensor::from_data_offsets_and_list_ids(
        query_data, 
        query_offsets,
        torch::empty({0, 1}, torch::TensorOptions().dtype(fvdb::JLIdxScalarType).device(query_data.device()))
    );
