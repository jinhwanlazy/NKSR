...
void forEachVoxelAnd1NeighborCPU(const fvdb::detail::GridBatchImpl& batchHdl, Func func, Args... args) {
    ...
    const auto jidx = fvdb::tensorAccessor<torch::kCPU, int32_t, 1>(batchHdl.jidx(false));

    for (int64_t elementIdx = 0; elementIdx < total_voxels; elementIdx++) {
        const int32_t batchIdx = jidx[elementIdx];
...
template <typename Func, typename... Args>
__global__ void forEachVoxelAnd1NeighborKernel(const fvdb::TorchRAcc32<int32_t, 1> jidx, Func func, Args... args) {
...
void forEachVoxelAnd1NeighborCUDA(const fvdb::detail::GridBatchImpl& batchHdl, Func func, Args... args) {
    ...
    const auto jidx = fvdb::tensorAccessor<torch::kCUDA, int32_t, 1>(batchHdl.jidx(false));
...
variable_list QgBuilding::forward(
    ...
    torch::Tensor indexer = torch::full({pts.element_count(), 27}, -1, opts.dtype(torch::kInt32));

    torch::Tensor qg;
    if (grad) {
        qg = torch::full({pts.element_count(), 27, 3}, 0.0, opts.dtype(pts.dtype()));
    } else {
        qg = torch::full({pts.element_count(), 27}, 0.0, opts.dtype(pts.dtype()));
    }
...
variable_list QgBuilding::backward(AutogradContext *ctx, variable_list grad_output) {
    ...
    auto pts = fvdb::JaggedTensor::from_data_offsets_and_list_ids(
        pts_data, 
        pts_joffsets,
        torch::empty({0, 1}, torch::TensorOptions().dtype(fvdb::JLIdxScalarType).device(pts_data.device()))
    );
