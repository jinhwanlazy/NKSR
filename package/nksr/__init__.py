...
from nksr.utils import split_into_chunks, get_device, Device, jagged_cat
...
class Reconstructor:
    ...
    def reconstruct(self,
        ...
        if self.hparams.geometry == 'kernel':
            ...
            normal_xyz = jagged_cat([svh.get_voxel_centers(d) for d in range(self.hparams.adaptive_depth)])
            normal_value = jagged_cat([feat.normal_features[d] for d in range(self.hparams.adaptive_depth)])
            normal_weight = self.hparams.solver.normal_weight * (self.hparams.voxel_size ** 2) / (normal_xyz.joffsets[1:] - normal_xyz.joffsets[:-1])
            solve_kwargs = {
                'pos_xyz': xyz, 'normal_xyz': normal_xyz, 'normal_value': -normal_value,
                'pos_weight': self.hparams.solver.pos_weight / (xyz.joffsets[1:] - xyz.joffsets[:-1]),
