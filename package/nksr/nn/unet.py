    ...
    def forward(self,
        ...
        for module, upsampler, struct_conv, normal_conv, basis_conv, udf_conv in \
            ...
            if module is not None:
                ...
                vdb_tensor = fvdb.jcat([enc_feat, vdb_tensor], dim=1)

                vdb_tensor = module(vdb_tensor)

            # Do structure inference.
            res.structure_features[feat_depth] = struct_conv(vdb_tensor).data

            if udf_conv is not None:
                res.udf_features[feat_depth] = udf_conv(vdb_tensor).data
            ...
            dec_ijk = decoder_tmp_svh.grids[feat_depth].ijk.rmask(exist_mask)
            decoder_svh.build_from_grid_coords(feat_depth, dec_ijk)

            vdb_tensor = self.padding(vdb_tensor, decoder_svh.grids[feat_depth])
            upsample_mask = decoder_svh.grids[feat_depth].fill_from_grid(
            ...
            if feat_depth < adaptive_depth and normal_conv is not None:
                res.normal_features[feat_depth] = normal_conv(vdb_tensor).data
            res.basis_features[feat_depth] = basis_conv(vdb_tensor).data
