...
def jwb(tensor: torch.Tensor, batch_idx: int, batch_size: int):
    ...
    return JaggedTensor.from_data_and_indices(
        tensor, 
        torch.full((tensor.size(0), ), batch_idx, dtype=torch.int32, device=tensor.device), 
        batch_size
    )


def jagged_cat(jt_list: list[JaggedTensor]) -> JaggedTensor:
    results = [[] for _ in range(len(jt_list[0]))]
    for jt in jt_list:
        for i, v in enumerate(jt):
            results[i].append(v.jdata)
    return JaggedTensor([torch.cat(v, dim=0) for v in results])
